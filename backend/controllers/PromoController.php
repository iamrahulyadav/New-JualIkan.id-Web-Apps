...
class PromoController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            $image = UploadedFile::getInstance($model, 'promo_image');
            $model->promo_image = 'frontend/web/img/' . $image->baseName. '.' . $image->extension;
            $image->saveAs("../../".$model->promo_image);
            if ($model->save()) {
    ...
    {
        $model = $this->findModel($id);
        $image_before = $model->promo_image;

        if ($model->load(Yii::$app->request->post())) {
            $image = UploadedFile::getInstance($model, 'promo_image');
            if ($image == null) {
                $model->promo_image = $image_before;
            }else {
                $model->promo_image = 'frontend/web/img/' . $image->baseName. '.' . $image->extension;
                $image->saveAs("../../".$model->promo_image);
            }
